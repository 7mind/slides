...
\frametitle{DI is outdated and doesn't compose with FP?}
  ...
  \item DI is unsafe
  \begin{itemize}
    \item \textit{``my program compiled but failed at runtime after a huge delay''}
  \end{itemize}
  \item DI doesn't work with FP
  \begin{itemize}
...
class MyAppProd[F[_, _]] extends ModuleDef {
...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \item Auto-Traits and Auto-Factories\footnotemark[1],
  \end{enumerate}
  \footnotetext[1]{Run-time or compile-time generation}
...
\frametitle{Garbage Collection for better and faster tests}
  \begin{enumerate}
    ...
    \item Creation takes microseconds, not like in Spring,
    ...
    \item $\Rightarrow$ Significant savings on test setup boilerplate.
  \end{enumerate}
\end{frame}
...
\frametitle{Garbage Collection for deployment: flexible monoliths}
  ...
  \begin{enumerate}
  \item Develop software components (\textit{Roles}\footnotemark[1]) separately, even in multirepo,
  ...
  \item $\Rightarrow$ \textit{substantial} development simplification: full platform environment can be started on a low-end machine with one command,
  \end{enumerate}
...
\begin{frame}[fragile]
\frametitle{Effectful creation}
dsafgsdfhsdgh TODO FIXME
\end{frame}

\begin{frame}[fragile]
\frametitle{Lifecycle}
  Global resources are a fact of life:
  \begin{enumerate}
  \item Connection pools, thread pools
  \item Servers and external endpoints
  \item Configurations, metrics, heartbeats
  \item External log sinks
  \end{enumerate}
  They must be managed by DI to create reusable components.
  They must be closed properly after integration tests.
  We must not set them up anew in every test.

  A myriad of global resources may be started by an application.
  Many of them will have to be started for integration tests.

  \begin{scalacode}
  import org.http4s.client._
  import org.http4s.client.blaze._

  class BlazeClientModule[F[_]: TagK: ConcurrentEffect] 
    extends ModuleDef {
    make[Client[F]].fromResource { ec: ExecutionContext => 
...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \item $\Rightarrow$ the Plan may be introspected, printed, executed in compile-time by a code generator or executed in run-time.
  \end{enumerate}
\end{frame}
